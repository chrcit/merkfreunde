...
const Layout: React.FC<Props> = ({children, className}) => {
    return (<>
        <div className="bg-heroBlue h-full w-full">
            ...
            <CloudShape 
            svgClasses="transform rotate-180 relative z-20 filter drop-shadow-lg bg-opacity-95 relative top-1" pathClasses="fill-current text-white"/>
            
            <motion.main className={`relative z-20 p-5 sm:px-10 md:px-15 lg:px-25 mx-auto bg-white bg-opacity-95 ${className}`}>
                {children}
            </motion.main>
